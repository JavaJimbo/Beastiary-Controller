 ...
 *      10-15-16: First version, disables encoder, uses RB5 as optocoupler sensor
 *      
 */
...
unsigned char optoChangeCounter = 0;
...
isr(void) {
    ...
    static unsigned char previousOptoState = 0x00;
    ...
    if (RBIF){
        ...
        PORTBreg = PORTB & 0b00100000;
        if (PORTBreg != previousOptoState) {
            previousOptoState = PORTBreg;
            optoChangeCounter++;
        }
        /* 
